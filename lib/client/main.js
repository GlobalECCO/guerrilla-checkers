...
require(["underscore", "../js/ICanHaz.min", "lib/checkers"], function(_, ICanHaz, Checkers) {
  ...
  var PULSING_GUIRRILLA_CLASS = "guerrilla_theme_guerrilla_glow selectable pulsing";
  ...
  function addSoldierPieceBehaviour(piece) {
    ...
    if (isSoldierPlayer()) {
      ...
      pieceOnBoard.onclick = function(event) {
  ...
  function createGuerrillaMove($moves, move) {
    ...
    var pieceGlow = $("<div class='" + PULSING_GUIRRILLA_CLASS + "'>");
  ...
  function createSoldierMove($moves, piece, position) {
    ...
    newPieceOnBoard.onclick = function(event) {
