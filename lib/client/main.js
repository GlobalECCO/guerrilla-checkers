...
require(["underscore", "../js/ICanHaz.min", "lib/checkers", "lib/client/tutorial"], function(_, ICanHaz, Checkers, Tutorial) {
  ...
  var GLOWING_SOLDIER_CLASS = "soldier_piece coin_theme_soldier coin_theme_soldier_ring";
  var PULSING_SOLDIER_CLASS = "soldier_piece coin_theme_soldier selectable pulsing coin_theme_soldier_ring";
  var SHADOW_SOLDIER_CLASS = "coin_theme_soldier_glow selectable pulsing";
  ...
  function setSelectedSoldierPiece(piece) {
    ...
    for (var positionKey in g_soldierPiecesOnBoard) {
      ...
      if (piece && getPositionKey(piece.position) === positionKey && $(otherPieceOnBoard).find('.coin_theme_soldier_ring').length && !$(otherPieceOnBoard).find('.pulsing').length) {
        deselect = true;
      }
      $(otherPieceOnBoard).children().remove();

      // for (var i = 0; i < otherPieceOnBoard.children.length; ++i) {
      //   if (otherPieceOnBoard.children[i].className === GLOWING_SOLDIER_CLASS) {
      //     otherPieceOnBoard.removeChild(otherPieceOnBoard.children[i]);
      //     var pieceGlow = document.createElement("div");
      //     pieceGlow.className = PULSING_SOLDIER_CLASS;
      //     otherPieceOnBoard.appendChild(pieceGlow);
      //     break;
      //   }
      // }
    }
    if (g_gameState.isSoldierTurn() && piece && !deselect) {
      ...
      if (pieceOnBoard) {
        while (pieceOnBoard.firstChild) {
          pieceOnBoard.removeChild(pieceOnBoard.firstChild);
        }
        var pieceGlow = $("<div class='" + GLOWING_SOLDIER_CLASS + "'>");
        $(pieceOnBoard).append(pieceGlow);
      }
    } else {
    ...
    if (piece && !deselect) {
      ...
      hideSoldierMoves();
      $('.soldier_piece').each(function() {
        var pieceGlow = $("<div class='" + PULSING_SOLDIER_CLASS + "'>");
        $(this).append(pieceGlow);
      });
    }
  }
  ...
  function addSoldierPieceBehaviour(piece) {
    ...
    if (isSoldierPlayer()) {
      if (g_gameState.isSoldierTurn()) {
        var pieceGlow = $("<div class='" + PULSING_SOLDIER_CLASS + "'>");
        $(pieceOnBoard).append(pieceGlow);
        $(pieceOnBoard).addClass("selectable");
      }

      $(pieceOnBoard).click(function(event) {
  ...
  function updateGuerrillaReserves() {
    ...
    if (isSoldierPlayer()) {
      ...
      for (var i = 0; i < starting_guerrillas; i++) {
        ...
        guerrilla.style.top = row*(HEIGHT + MARGIN_TOP) + "px";
        guerrilla.style.left = (i % pieces_per_row) * (WIDTH + MARGIN_RIGHT) + "px";
      ...
      for (var i = 0; i < num_reserves; i++) {
        ...
        reserve.style.top = row*(HEIGHT + MARGIN_TOP) + "px";
        reserve.style.left = (i % pieces_per_row) * (WIDTH + MARGIN_RIGHT) + Math.floor((row+1) / 7)*LAST_ROW_OFFSET + "px";
  ...
  function createGuerrillaMove($moves, move) {
    ...
    var css_class = g_showShadows ? "guerrilla_move guerrilla_shadow" : "guerrilla_move";
    var newPieceOnBoard = addPiece(container, piece, css_class, GUERRILLA_MARGIN);

    $(newPieceOnBoard).click(function() {
      emit('placeGuerrilla', piece);
    });

    //if move shadows are turned off, then we'll just display them on mouse-over
    //instead of all the time.
    if (!g_showShadows) {
      $(newPieceOnBoard).hover(function(event) {
        $(this).addClass("guerrilla_shadow");
      }, function(event) {
        $(this).removeClass("guerrilla_shadow");
      });
    }
  }
  ...
  function createSoldierMove($moves, piece, position) {
    ...
    var container = $moves.get(0);
    var css_class = g_showShadows ? 'soldier_move soldier_shadow '+cssTheme()+"_soldier" : "soldier_move";
    var newPieceOnBoard = addPiece(container, move, css_class, SOLDIER_MARGIN);

    if (g_showShadows) {
      //create a glowing child element
      var pieceGlow = $("<div class='" + SHADOW_SOLDIER_CLASS + "'>");
      $(newPieceOnBoard).append(pieceGlow);
    }
    else{
      //otherwise only display the glowing child during a mouse-hover
      $(newPieceOnBoard).hover(function(event) {
          $(this).addClass('soldier_shadow '+cssTheme()+"_soldier");
          var pieceGlow = $("<div class='" + SHADOW_SOLDIER_CLASS + "'>");
          $(this).append(pieceGlow);
        }, function(event){
          $(this).removeClass('soldier_shadow '+cssTheme()+"_soldier");
          $(this).empty();
        });
    }

    $(newPieceOnBoard).click(function(event) {
      emit('moveCOIN', move);
      setSelectedSoldierPiece(null);
      hideSoldierMoves();
      $('.soldier_piece').children().remove();
      event.stopPropagation();
    });
  }
  ...
  function onSocketUserData(data) {
    $('#username').val(data.username);
    g_role = data.role;
    if (isGuerrillaPlayer()) {
      printMessage("server", "You are the Guerrilla player!", 'server');
      $('.board').addClass('guerrilla_board');
      $('#side').addClass('guerrilla_side');
    } else if (isSoldierPlayer()) {
      printMessage("server", "You are the State player!", 'server');
  ...
  function onSocketUpdate(updateResponse, fromTutorial) {
    ...
    // If the game just started and the tutorial hasn't been seen yet, show it now
    if (((isSoldierPlayer() && g_gameState.turnCount <= 1) || g_gameState.turnCount === 0) && !Tutorial.isActive() && !Tutorial.isFinished()) {
      Tutorial.setInitialGameState(g_gameState);
      Tutorial.beginTutorial();
    }
    // If the tutorial is still up and the other player took their turn, store it for later
    else if (Tutorial.isActive() && !Tutorial.isFinished() && fromTutorial !== true) {
      Tutorial.setInitialGameState(g_gameState);
    }
    // Otherwise, show the game state
    else {
      notifyPlayer();
...
//  });
  ...
  $("#checkers").bind('click', function() {
    if (isSoldierPlayer() && !g_gameState.movedSoldier && g_gameState.isSoldierTurn()) {
      hideSoldierMoves();
      $('.soldier_piece').each(function() {
        var pieceGlow = $("<div class='" + PULSING_SOLDIER_CLASS + "'>");
        $(this).append(pieceGlow);
  ...
  Tutorial.init(socket, isSoldierPlayer, onSocketUpdate, tutorialEnd);

  function tutorialEnd() {
    setSelectedSoldierPiece(null);
    $('#pieces').empty();
    g_soldierPiecesOnBoard = {};
    g_guerrillaPiecesOnBoard = {};
  }
});
